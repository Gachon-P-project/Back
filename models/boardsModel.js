...
exports.readList = (user_no, subject_name, professor_name, cb) => {
    ...
    const sql = "select b.*, (select nickname from USER where user_no = b.user_no) as nickname, (select count(*) from REPLY where post_no=b.post_no) as reply_cnt, (select count(*) from LIKEBOARD where post_no=b.post_no and board_flag = 1) as like_cnt, (select count(*) from LIKEBOARD where user_no=? and post_no=b.post_no and board_flag = 1) as like_user from BOARD b where b.subject_name=? and b.professor_name=? order by post_no desc";
