...
for(i=0; i<=14; i++) {
    ...
    for(j=1; j<=major[i].size; j++) {
        ...
        const url = "http://sg.gachon.ac.kr/main?attribute=timeTableJson&lang=ko&year="+year+"&hakgi="+hakgi+"&menu=1&p_isu_cd="+p_isu_cd+"&p_univ_cd=CJ0000&p_maj_cd="+p_maj_cd+"&p_cor_cd="+p_cor_cd;
        ...
        .then(result => {
            ...
            const sql = "INSERT INTO PROFESSOR_copy SET ?"
            dataObject.map(data => {
                db.query(sql, data, (err, result) => {
                    if(err) console.log("insert err : ", sql, data);
                    else console.log("insert result : ", sql, data);
                })
            })
        })
    }
